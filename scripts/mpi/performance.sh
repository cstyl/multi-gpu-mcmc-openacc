  ...
  do
    ...
    do
      ...
      do
        ...
        outdir="./mpi-${N}_${dim}_${p}/$k"
        # generate input file for the test
        infile="mpi-${N}_${dim}_${p}_${k}.dat"
        outfile="mpi-${N}_${dim}_${p}_${k}.txt"
        echo "nnodes        1" > $infile
        echo "nprocs        ${p}" >> $infile
        ...
        then
          ...
          $QSUB -N "${p}_${k}_${N}_${dim}" \
                -v INFILE="$infile",OUTFILE="$outfile",OUTDIR="$outdir",EXECDIR="$EXEC_DIR",PROCS="$p" \
                submit.pbs
        else
