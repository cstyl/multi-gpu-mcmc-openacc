...
echo "Starting Single-GPU Correctness test..." 2>&1 | tee $outfile
for N in $DATAPOINTS
do
  for dim in $DIMS
  do
    ...
    do
      ftest="${EXEC_DIR}/single-gpu-${N}_${dim}/$k/progress.csv"
      if [ -f $ftest ];
      then
...
echo "Single-GPU Correctness test completed. Results in $outfile" 2>&1 | tee -a $outfile
