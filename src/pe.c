...
int pe_free(pe_t * pe) {
  ...
  if (pe->nref <= 0) {
    ...
    if (pe->unquiet) pe_info(pe, "MCMC finished normally.\n");
    free(pe);
    pe = NULL;
  }
...
int pe_message(pe_t * pe) {
  ...
  pe_info(pe,
       "Welcome to MCMC sampler v%d.%d.%d (%s version running on %d process%s)\n\n",
       MCMC_MAJOR_VERSION, MCMC_MINOR_VERSION, MCMC_PATCH_VERSION,
